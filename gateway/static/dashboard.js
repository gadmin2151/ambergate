...
function dashboardContent() {
  const stale = dashboardError ? `<div class="dash-alert danger" role="alert">${icon('info')}<div><strong>Обновление данных недоступно</strong><p>${esc(dashboardError)}.${dashboardData ? ` Показан последний снимок на ${dashTime(dashboardData.updated_at)}.` : ' Нажмите «Обновить», чтобы повторить.'}</p></div></div>` : '';
  if (!dashboardData) return stale || `<div class="dash-loading" role="status">${icon('dashboard')}<strong>Подключаемся к вашему gateway</strong><span>Загружаем состояние Nginx и статистику трафика…</span></div>`;
  ...
  const statusTitle = !d.nginx.healthy ? 'Проверьте состояние Nginx' : s.errors ? 'Gateway работает · есть ошибки 5xx' : 'Gateway в работе';
  ...
  return `${stale}
    ...
    <div class="dash-kpis">
      ...
      ${dashMetric('Ошибки сервера', dashNumber(s.errors), '5xx', s.requests ? `<b>${dashNumber(s.error_percent)}%</b> от всех запросов` : 'Пока нет завершённых запросов', 'info', s.errors ? 'has-errors' : '')}
      ${dashMetric('Время ответа · p95', dashNumber(s.p95_ms), 'мс', s.requests ? `Среднее <b>${dashNumber(s.avg_ms)} мс</b> · p95 приблизительно` : 'Измеряется при получении трафика', 'clock')}
      ${dashMetric('Попадания в кеш', dashNumber(s.cache_hit_percent), s.cache_hit_percent == null ? '' : '%', s.cache_lookups ? `<b>${dashNumber(s.cache_hits)}</b> HIT из ${dashNumber(s.cache_lookups)} обращений` : 'Ещё нет обращений к кешу', 'cache')}
    </div>
    <div class="dash-primary-grid">
      <section class="dash-panel dash-traffic"><div class="dash-panel-head"><div><h2>Трафик через gateway</h2><p>Завершённые запросы · по 10 с · последний интервал неполный</p></div><span class="dash-transfer">${icon('download')} ${dashBytes(s.bytes)}<small>отдано клиентам</small></span></div>
        <div class="dash-chart-legend"><span><i class="amber-dot"></i>Все запросы</span><span><i class="red-dot"></i>Ошибки 5xx</span><span class="dash-chart-period">${elapsed < 900 ? `Собрано за ${dashDuration(elapsed)}` : 'Последние 15 минут'}</span></div>
        ${trafficChart(d)}
    ...
    ${eventsPanel(d)}
    <div class="dash-footnote"><span>${icon('lock')}Метрики локальны · история в памяти до 15 минут</span><span>Сбор с ${new Date(d.collected_since * 1000).toLocaleString('ru-RU')} · сброс при перезапуске</span></div>`;
}
...
function trafficChart(d) {
  ...
  const chartTime = stamp => new Date(stamp * 1000).toLocaleTimeString('ru-RU', {hour:'2-digit', minute:'2-digit', ...(end-start < 120 ? {second:'2-digit'} : {})});
  ...
  const targets = points.map(p => `<rect x="${Math.max(left, x(p.time) - (right-left) / Math.max(points.length, 1) / 2)}" y="${top}" width="${(right-left) / Math.max(points.length, 1)}" height="${bottom-top}" class="dash-chart-target"><title>${chartTime(p.time)} — ${p.requests} запросов, ${p.errors} ошибок 5xx${p.time + 10 > d.updated_at ? " · интервал ещё собирается" : " (10 с)"}</title></rect>`).join('');
  return `<div class="dash-chart ${d.summary.requests ? '' : 'no-traffic'}"><svg viewBox="0 0 ${w} 228" role="img" aria-label="Трафик: ${d.summary.requests} запросов и ${d.summary.errors} ответов 5xx за доступный период"><defs><linearGradient id="traffic-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#ffaf35" stop-opacity=".24"/><stop offset="100%" stop-color="#ffaf35" stop-opacity=".015"/></linearGradient><clipPath id="traffic-clip"><rect x="${left}" y="0" width="${right-left}" height="${bottom+1}"/></clipPath></defs>${grid}${labels}<g clip-path="url(#traffic-clip)"><path d="${area}" fill="url(#traffic-fill)"/><path d="${path('requests')}" class="dash-traffic-line"/><path d="${path('errors')}" class="dash-error-line"/>${points.length === 1 && points[0].requests ? `<circle cx="${left + 2}" cy="${y(points[0].requests)}" r="3" fill="#ffaf35"/>` : ''}${targets}</g></svg>${d.summary.requests ? '' : `<div class="dash-chart-empty"><span>${icon('bolt')}</span><strong>Здесь появится ваш трафик</strong><p>Запросы к доменам gateway заполнят график автоматически.</p></div>`}</div>`;
}
function systemPanel(d, hosts, routes) {
  const c = d.connections, cfg = d.configuration;
  return `<section class="dash-panel dash-system"><div class="dash-panel-head"><div><h2>Состояние Nginx</h2><p>Текущий момент</p></div><span class="dash-small-icon">${icon('server')}</span></div><div class="dash-connection"><strong>${dashNumber(c?.active)}</strong><span>открытых<br>соединений</span><span class="dash-connection-mark">${icon('bolt')}</span></div><div class="dash-connection-parts"><div><strong>${dashNumber(c?.reading)}</strong><span>Чтение</span></div><div><strong>${dashNumber(c?.writing)}</strong><span>Ответ</span></div><div><strong>${dashNumber(c?.waiting)}</strong><span>Keep-alive</span></div></div><dl class="dash-facts"><div><dt>Время работы</dt><dd>${dashDuration(d.nginx.uptime)}</dd></div><div><dt>Действующие домены</dt><dd>${hosts.length}<span> / ${cfg.hosts.length}</span></dd></div><div><dt>Маршруты / цели</dt><dd>${routes.length} / ${routes.reduce((n, r) => n + r.targets.length, 0)}</dd></div><div><dt>Глобальный лимит / IP</dt><dd>${cfg.settings.rate_rps ? cfg.settings.rate_rps + ' запр/с' : 'Без лимита'}</dd></div></dl><div class="dash-version"><span><i class="dot ${d.nginx.healthy ? '' : 'bad'}"></i> Версия <code>${esc(cfg.generation.slice(0, 8))}</code></span>${dashLink('history', 'История', 'dash-history-link')}</div>${d.nginx.last_error ? `<p class="dash-runtime-error">Последнее применение не удалось. Откройте nginx.conf и проверьте конфигурацию.</p>` : ''}</section>`;
}
...
function responsesPanel(s) {
  ...
  return `<section class="dash-panel dash-responses"><div class="dash-panel-head"><div><h2>HTTP-ответы</h2><p>Распределение кодов</p></div>${icon('code')}</div><div class="dash-response-content"><div class="dash-donut"><svg viewBox="0 0 150 150" aria-hidden="true"><circle cx="75" cy="75" r="61" class="dash-ring-track"/><g transform="rotate(-90 75 75)">${arcs}</g></svg><div><strong>${success == null ? '—' : dashNumber(success) + '%'}</strong><span>2xx + 3xx</span></div></div><div class="dash-response-legend">${[['1xx','Информация'],['2xx','Успешные'],['3xx','Редиректы'],['4xx','Ошибки клиента'],['5xx','Ошибки сервера']].map(([code, label], i) => `<div><i class="response-${i+1}"></i><span><b>${code}</b> ${label}</span><strong>${dashNumber(s.statuses[i])}</strong></div>`).join('')}</div></div><div class="dash-rate-limits"><span>${icon('shield')} Лимиты gateway · HTTP 429</span><strong>${dashNumber(s.limited)}</strong></div></section>`;
}
function eventsPanel(d) {
  return `<section class="dash-panel dash-events"><div class="dash-panel-head"><div><h2>Последние проблемные ответы <span class="dash-count">${d.events.length}</span></h2><p>До 30 последних ответов 4xx и 5xx за доступный период</p></div><span class="dash-event-label">${icon('info')} По реальным запросам</span></div>${d.events.length ? `<div class="dash-table-scroll" id="dash-event-scroll" data-preserve-scroll><table class="dash-table"><thead><tr><th>Время</th><th>Домен / маршрут</th><th>Ответ</th><th>Длительность</th></tr></thead><tbody>${d.events.map(e => `<tr><td class="dash-event-time">${new Date(e.time*1000).toLocaleTimeString('ru-RU')}</td><td><strong>${esc(dashHost(e.host))}</strong><code class="dash-event-route">${esc(e.route === '-' ? 'без upstream' : e.route)}</code></td><td><span class="dash-code ${e.status >= 500 ? 'is-server-error' : ''}">${e.status}</span><span class="dash-error-reason">${e.limited ? 'Лимит gateway' : ({429:'Слишком много запросов',502:'Ошибка upstream',503:'Сервис недоступен',504:'Тайм-аут',404:'Не найдено',403:'Доступ запрещён',499:'Клиент закрыл соединение'})[e.status] || (e.status >= 500 ? 'Ошибка сервера' : 'Ошибка запроса')}</span></td><td>${dashNumber(e.ms)} мс</td></tr>`).join('')}</tbody></table></div>` : `<div class="dash-events-empty"><span>${icon('check')}</span><div><strong>${d.summary.requests ? 'Ответов с ошибками пока нет' : 'Журнал готов к работе'}</strong><p>${d.summary.requests ? 'В собранных запросах не обнаружены ответы 4xx или 5xx.' : 'При появлении ответов 4xx или 5xx здесь будут домен, маршрут и время.'}</p></div></div>`}</section>`;
}
