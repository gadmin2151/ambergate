...
function dashboardContent() {
  ...
  if (!dashboardData) return stale || ui`<div class="dash-loading" role="status">${icon('dashboard')}<strong>Подключаемся к вашему gateway</strong><span>Загружаем состояние Nginx и статистику трафика…</span></div>`;
  ...
  const statusTitle = !d.nginx.healthy ? t('Проверьте состояние Nginx') : s.errors ? t('Gateway работает · есть ошибки 5xx') : t('Gateway в работе');
