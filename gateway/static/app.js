...
function setModal(title, subtitle, content, submit, button=t('Сохранить')) {
  ...
  modal.innerHTML = ui`<form id="modal-form"><div class="modal-header"><div><span class="eyebrow">НАСТРОЙКИ GATEWAY</span><h2 id="modal-title">${esc(title)}</h2><p class="modal-subtitle">${subtitle}</p></div>${btn('close','', 'close','ghost icon',t('aria-label="Закрыть"'))}</div><div class="modal-body">${content}<div id="modal-error" class="error" role="alert"></div></div><div class="form-actions"><span class="hint">${button===t('Изменить пароль') ? t('Потребуется повторный вход') : t('Изменения попадут в черновик')}</span>${btn('close',t('Отмена'),'','ghost')}<button type="submit" class="primary">${icon('check')}${esc(button)}</button></div></form>`;
...
function render() {
  ...
  const descriptions = {
    dashboard:t('Трафик, ответы и состояние вашего gateway — в реальном времени.'),
  ...
  $('#app').innerHTML = ui`
    <div class="shell ${page === 'dashboard' ? 'dashboard-shell' : ''}">
      <aside class="sidebar">
        ...
        <div class="workspace-switch"><span class="workspace-symbol">${icon('server')}</span><div><strong>Мой gateway</strong><small>${t('Self-hosted workspace')}</small></div><span class="workspace-dot"></span></div>
      ...
      <div class="workspace">
        <header class="topbar"><div class="crumb">${icon('server')}<span>Мой gateway</span>${icon('chevron')}<strong>${t(current[1])}</strong></div><div class="top-right">${languageSwitcher()}<span class="status" id="nginx-status">${statusHTML()}</span><span class="topbar-divider"></span>${btn('logout','','exit','ghost icon',t('aria-label="Выйти" title="Выйти"'))}</div></header>
        <main><div class="page-head"><div><span class="eyebrow">${page === 'dashboard' ? t('LIVE OVERVIEW') : page === 'routes' ? t('TRAFFIC MANAGEMENT') : t('GATEWAY CONTROL')}</span><h1>${page === 'routes' ? t('Домены и маршруты') : t(current[1])}</h1><p>${descriptions[page]}</p></div>${page === 'dashboard' ? ui`<div class="dash-controls"><span>${icon('clock')} Последние 15 минут</span>${btn('refresh-dashboard',t('Обновить'),'history','', 'id="refresh-dashboard"')}</div>` : page === 'routes' ? btn('add-host',t('Добавить домен'),'plus','primary') : `<span class="page-symbol">${icon(page)}</span>`}</div>
