...
function dockerStatus() {
  ...
  return badge(dockerData.connected ? 'Подключён' : dockerData.settings.enabled ? 'Нет подключения' : 'Выключен', dockerData.connected ? 'green' : dockerData.settings.enabled ? 'amber' : 'gray');
}
function dockerConnectionActions() {
  return `${dockerData?.settings.enabled ? btn('docker-disconnect','Отключить','','ghost small',dockerLoading ? 'disabled' : '') : ''}${btn('docker-connect',dockerLoading ? 'Подключаемся…' : dockerData?.settings.enabled ? 'Сохранить и проверить' : 'Подключить Docker','docker','primary',dockerLoading || !dockerData ? 'disabled' : '')}`;
}
...
function dockerInventory() {
  if (!dockerData) return `<div class="docker-empty">${icon('docker')}<h3>Загружаем настройки Docker…</h3></div>`;
  const d = dockerData;
  if (!d.connected) return `<div class="docker-empty">${icon('docker')}<h3>${d.settings.enabled ? 'Docker пока недоступен' : 'Подключите ваши контейнеры'}</h3><p>${esc(d.message)}</p><p>После подключения здесь появятся контейнеры, сети и TCP-порты.</p></div>`;
  return `<div class="docker-summary"><div>${icon('docker')}<span>Engine <strong>${esc(d.engine_version)}</strong></span></div><div>${icon('server')}<span><strong>${d.containers.length}</strong> контейнеров · <strong>${d.containers.filter(c=>c.selectable).length}</strong> для выбора</span></div><div>${icon('routes')}<span>${d.mode === 'container' ? 'Сети gateway: ' + esc(d.gateway_networks.join(', ') || 'не определены') : 'Локальный режим · опубликованные порты'}</span></div></div><div class="docker-inventory-heading"><div><h2>Контейнеры</h2><p>${esc(d.message)} · обновлено ${dockerDate(d.checked_at)}</p></div>${btn('docker-refresh','Обновить','history','small',dockerLoading ? 'disabled' : '')}</div>${d.truncated ? '<p class="docker-info">Показаны последние 500 контейнеров Docker.</p>' : ''}<div class="docker-filters"><label class="search-field">${icon('search')}<input id="docker-search" type="search" placeholder="Контейнер, образ или Compose-сервис…" aria-label="Поиск Docker-контейнеров" value="${esc(dockerQuery)}"></label><label class="docker-filter-network"><select id="docker-network" aria-label="Сеть Docker"><option value="">Все сети</option>${d.networks.map(n=>`<option value="${esc(n)}" ${dockerNetwork===n?'selected':''}>${esc(n)}</option>`).join('')}</select></label>${check('Можно выбрать','docker-ready',dockerOnlyReady)}</div><div id="docker-container-list">${dockerContainerList()}</div><div class="docker-route-tip">${icon('routes')}<div><strong>Выберите контейнеры в настройках маршрута</strong><p>Домены и маршруты → нужный маршрут → Серверы → Выбрать из Docker.</p></div>${btn('navigate','К маршрутам','arrow','small','data-page="routes"')}</div>`;
}
function dockerContainerList() {
  const rows = dockerFiltered(dockerData,dockerQuery,dockerNetwork,dockerOnlyReady);
  if (!rows.length) return `<div class="docker-empty compact"><h3>Контейнеров не найдено</h3><p>${dockerData.containers.length ? 'Измените поиск или фильтр сети.' : 'Запустите приложения в Docker и обновите список.'}</p></div>`;
  return `<div class="docker-table-wrap"><table class="docker-table"><thead><tr><th>Контейнер / образ</th><th>Состояние</th><th>TCP-порты</th><th>Сети</th><th>Маршрутизация</th></tr></thead><tbody>${rows.map(c=>`<tr><td><strong>${esc(c.name)}</strong><small>${esc(c.image)}</small>${c.project ? `<span class="docker-compose-tag">${esc(c.project)} / ${esc(c.service)}</span>` : ''}</td><td>${badge(dockerState(c.state),c.state==='running'?'green':'gray')}<small>${esc(c.status)}</small></td><td><code>${c.ports.join(', ') || 'Не объявлены'}</code></td><td>${c.networks.map(n=>`<span class="docker-network-tag ${c.shared_networks.includes(n)?'shared':''}">${esc(n)}</span>`).join('') || '—'}</td><td>${c.selectable ? `<span class="docker-eligible">${icon('check')}${c.endpoints[0].kind==='dns'?'Имя в Docker DNS':c.endpoints[0].kind==='published'?'Порт хоста':'IP в общей сети'}</span><code>${esc(dockerAddress(c.endpoints[0].address,c.endpoints[0].port))}</code>` : `<span class="docker-unavailable">${esc(c.reason)}</span>`}</td></tr>`).join('')}</tbody></table></div>`;
}
async function loadDockerPage(force=false) {
  ...
  } catch(error) {if(csrf){dockerError=error instanceof TypeError?'Не удалось связаться с Gateway':error.message;}}
...
function updateDockerPage() {
  ...
  $('#docker-connection-error').textContent=dockerError;
...
async function openDockerPicker(refresh=false) {
  ...
  if (!refresh) {
    dialog.innerHTML=`<div class="modal-header"><div><span class="eyebrow">DOCKER DISCOVERY</span><h2 id="docker-picker-title">Контейнеры для маршрута</h2><p class="modal-subtitle">Выберите приложения и TCP-порты. Несколько целей распределят нагрузку.</p></div>${btn('docker-picker-close','','close','ghost icon','aria-label="Закрыть выбор контейнеров"')}</div><div id="docker-picker-body" class="modal-body"><p class="docker-loading">Подключаемся к Docker…</p></div>`;
    dialog.showModal();
  } else $('#docker-picker-body').innerHTML='<p class="docker-loading">Обновляем контейнеры…</p>';
  try {
    ...
    if(!data.connected) {
      $('#docker-picker-body').innerHTML=`<div class="docker-empty compact">${icon('docker')}<h3>${data.settings.enabled ? 'Нет подключения к Docker' : 'Docker не подключён'}</h3><p>${esc(data.message)}</p><p>Подключите socket в разделе Docker. Вы можете закрыть выбор и продолжить редактирование вручную.</p>${btn('docker-picker-close','Вернуться к маршруту','arrow','primary')}</div>`;
      return;
    }
    renderDockerPicker();
    ...
    if(dialog.open && token===dockerPickerToken && csrf)$('#docker-picker-body').innerHTML=`<div class="error">${esc(error instanceof TypeError ? 'Не удалось связаться с Gateway' : error.message)}</div>${btn('docker-picker-refresh','Повторить','history','small')}`;
  }
}
...
function updateDockerSelection(row) {
  ...
  $('#docker-selection-count').textContent=`Выбрано контейнеров: ${count}`;
...
function addDockerTargets(event) {
  ...
  if(!result.length||result.length>32){$('#docker-picker-error').textContent='В маршруте должно быть от 1 до 32 серверов. При необходимости замените текущий список.';return;}
  editor.innerHTML=result.map(targetFields).join('');
  $('#modal').dataset.replaceDockerTargets='false';
  $('#docker-picker').close();
  notify('Контейнеры добавлены в редактор. Сохраните маршрут, затем примените конфигурацию.');
}
