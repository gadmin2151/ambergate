...
const nav = [['dashboard','Обзор системы'],['routes','Маршруты'],['docker','Docker'],['agents','Агенты'],['shield','Защита и лимиты'],['cache','Кеширование'],['code','nginx.conf'],['history','История версий'],['general','Общие настройки']];
...
async function api(path, body, options = {}) {
  ...
  if (!response.ok) {
    ...
    const error=new Error(data.error || `HTTP ${response.status}`);error.code=data.code;error.details=data;throw error;
  }
  return data;
}
function notify(message, error = false) {
  ...
  const toast = $('#toast');
  toast.innerHTML = `${icon(error ? 'info' : 'check')}<span>${esc(translateError(message))}</span>${btn('dismiss-toast', '', 'close', 'ghost icon', t('aria-label="Закрыть уведомление"'))}`;
...
function updateSaveState() {
  ...
  if (dock) {dock.innerHTML = saveDockContent(); dock.hidden = ['dashboard','docker','agents','general'].includes(page) && !dirty && !state.pending;}
...
function setModal(title, subtitle, content, submit, button=t('Сохранить')) {
  const modal = $('#modal');
  ...
  modal.innerHTML = ui`<form id="modal-form"><div class="modal-header"><div><span class="eyebrow">НАСТРОЙКИ AMBERGATE</span><h2 id="modal-title">${esc(title)}</h2><p class="modal-subtitle">${subtitle}</p></div>${btn('close','', 'close','ghost icon',t('aria-label="Закрыть"'))}</div><div class="modal-body">${content}<div id="modal-error" class="error" role="alert"></div></div><div class="form-actions"><span class="hint">${button===t('Изменить пароль') ? t('Потребуется повторный вход') : t('Изменения попадут в черновик')}</span>${btn('close',t('Отмена'),'','ghost')}<button type="submit" class="primary">${icon('check')}${esc(button)}</button></div></form>`;
  ...
  $('#modal-form').addEventListener('submit', async event => {
    ...
    catch (error) { $('#modal-error').textContent = translateError(error.message); }
    finally {button.disabled = false;}
  });
  if (!modal.open) modal.showModal();
}
function confirmAction(title, description, detail, label=t('Подтвердить'), danger=false) {
  const dialog = $('#confirm-modal');
  if (dialog.open) return Promise.resolve(false);
  dialog.returnValue = 'cancel';
  dialog.innerHTML = ui`<form method="dialog"><div class="confirm-content"><span class="confirm-symbol">${icon(danger ? 'trash' : 'info')}</span><h2 id="confirm-title">${esc(title)}</h2><p id="confirm-description">${esc(description)}</p>${detail ? `<div class="confirm-detail">${esc(detail)}</div>` : ''}<p class="confirm-hint">${danger ? t('После удаления сохраните черновик или примените изменения. ') : ''}Рабочая конфигурация обновится после «Применить».</p></div><div class="form-actions"><button type="submit" value="cancel" autofocus>Отмена</button><button type="submit" value="confirm" class="${danger ? 'confirm-delete' : 'primary'}">${icon(danger ? 'trash' : 'check')}${esc(label)}</button></div></form>`;
...
function switchLanguage(value) {
  if (busy || value === language || document.querySelector('dialog[open]')) return;
  const focused = document.activeElement;
  // Preserve even incomplete form values; rendering must not save or validate a draft.
  const inputs = [...document.querySelectorAll('#app input, #app textarea, #app select')].map(el => ({id:el.id, name:el.name, value:el.value, checked:el.checked, focus:el===focused}));
  const scroll = [window.scrollX, window.scrollY];
  setLanguage(value);
  $('#toast').hidden = true;
  if (config && csrf) render(); else showLogin();
  for (const saved of inputs) {
    const el = saved.id ? document.getElementById(saved.id) : [...document.querySelectorAll('#app input, #app textarea, #app select')].find(el=>el.name===saved.name);
    if (el) {el.value=saved.value; el.checked=saved.checked; if(saved.focus) el.focus({preventScroll:true});}
  }
...
function showLogin() {
  ...
  $('#app').innerHTML = ui`<div class="login-page"><section class="login-art"><div class="brand"><img src="/favicon.svg?v=3" alt=""><div>Amber<span class="brand-accent">Gate</span><small>NGINX GATEWAY</small></div></div><div><div class="eyebrow">Один вход. Все приложения.</div><h1>Все приложения.<br><span>Один AmberGate.</span></h1><div class="login-flow"><span>example.com</span><i></i><div><code>/</code><code>/api</code><code>/s3</code></div></div><p>Маршруты, балансировка и защита приложений — в одной панели управления Nginx.</p></div><div class="eyebrow">${t('Self-hosted · Local configuration · HTTP gateway')}</div></section><section class="login-form-wrap">${languageSwitcher()}<form id="login" class="login-form"><div class="mobile-brand"><img src="/favicon.svg?v=3" alt="AmberGate"></div><div class="eyebrow">Панель управления</div><br><h2>Добро пожаловать</h2><p>Войдите, чтобы настроить ваш AmberGate.</p>${field(t('Пароль администратора'),'password','','password','required autocomplete="current-password" autofocus')}<div class="error" id="login-error"></div><button type="submit" class="primary">Войти в AmberGate ${icon('arrow')}</button><p class="hint">При первом запуске пароль задаётся через AMBERGATE_ADMIN_PASSWORD или выводится в логах контейнера.</p></form></section></div>`;
  $('#login').addEventListener('submit', async event => {
    ...
    catch(error){const el = $('#login-error'); if(el) el.textContent = translateError(error.message);}
    finally{button.disabled = false;}
  });
}
async function load(){accept(await api('config')); render(); startDashboardStream();}
function statusHTML() {
  if (dashboardStreamState !== 'live') return `<span class="dot bad"></span>${dashboardError ? t('Нет live-связи') : t('Подключаемся…')}`;
  return `<span class="dot ${health?.healthy ? '' : 'bad'}"></span>${health?.healthy ? t('Nginx работает') : t('Nginx недоступен')}`;}
function saveDockContent() {
  const pending = dirty || state.pending;
  return `<div class="save-state ${pending ? 'is-pending' : ''}"><span class="save-state-icon">${icon(pending ? 'clock' : 'check')}</span><div><strong>${busy ? t('Проверяем конфигурацию…') : dirty ? t('Есть несохранённые изменения') : state.pending ? t('Черновик готов к применению') : t('Все изменения применены')}</strong><span>${pending ? t('Рабочие маршруты обновятся после применения') : t('Nginx использует последнюю сохранённую версию')}</span></div></div><div class="actions">${btn('save',t('Сохранить черновик'),'save','',!dirty || busy ? 'disabled' : '')}${btn('apply',busy ? t('Применяем…') : t('Применить'),busy ? '' : 'arrow','primary',(!dirty && !state.pending) || busy ? 'disabled' : '')}</div>`;
}
function render() {
  ...
  const descriptions = {
    ...
    general:at('Общий адрес сервера и параметры подключения агентов.','Server address and agent connection settings.'),
    agents:at('Контейнеры на удалённых машинах — через исходящие туннели.','Containers on remote machines, connected through outbound tunnels.'),
    ...
    cache:t('Ускоряйте публичные ответы и снижайте нагрузку на серверы.'),
    code:t('Вся конфигурация перед вами. Проверьте её перед применением.'),
    history:t('Вернитесь к предыдущей конфигурации в несколько кликов.')
  };
  $('#app').innerHTML = ui`
    <div class="shell ${page === 'dashboard' ? 'dashboard-shell' : ''}">
      ...
      <div class="workspace">
        ...
        <main><div class="page-head"><div><span class="eyebrow">${page === 'dashboard' ? t('LIVE OVERVIEW') : page === 'routes' ? t('TRAFFIC MANAGEMENT') : t('AMBERGATE CONTROL')}</span><h1>${page === 'routes' ? esc(config.hosts.find(h=>h.id===selectedHost)?.domain || t('Домены и маршруты')) : t(current[1])}</h1><p>${descriptions[page]}</p></div>${page === 'dashboard' ? ui`<div class="dash-controls"><span>${icon('clock')} Последние 15 минут</span>${btn('refresh-dashboard',t('Обновить'),'history','', 'id="refresh-dashboard"')}</div>` : page === 'routes' ? (selectedHost ? btn('edit-host',t('Настройки домена'),'sliders','',`data-host="${selectedHost}"`) : btn('add-host',t('Добавить домен'),'plus','primary')) : `<span class="page-symbol">${icon(page)}</span>`}</div>
          ...
          <div class="page-note"><span>${icon('lock')}Локальное хранение конфигурации</span><span>AmberGate <span class="footer-version">v0.1.0</span></span></div>
        </main>
...
function routesPage() {
  ...
  return ui`<div class="metrics">${metrics.map(([symbol,label,value,note,tone])=>`<div class="metric"><div class="metric-top"><span>${label}</span><span class="metric-icon ${tone}">${icon(symbol)}</span></div><div class="metric-value">${value}</div><div class="metric-note">${note}</div></div>`).join('')}</div>
    <div class="section-title"><div><h2>Ваши домены <span class="count">${config.hosts.length}</span></h2><p>Отдельные маршруты и серверы для каждого домена.</p></div><div class="actions">${btn('import',t('Импорт'),'upload','ghost small')}${btn('export',t('Экспорт'),'download','ghost small')}</div></div>
    <div class="route-toolbar"><label class="search-field">${icon('search')}<input id="route-search" type="search" placeholder="Найти домен, путь или сервер…" aria-label="Поиск доменов и маршрутов" value="${esc(routeQuery)}"><kbd>/</kbd></label><div class="filter-tabs" role="group" aria-label="Фильтр доменов">${[['all',t('Все')],['enabled',t('Включены')],['disabled',t('Выключены')]].map(([key,label])=>btn('filter-hosts',label,'',hostFilter===key ? 'selected' : '',`data-filter="${key}" aria-pressed="${hostFilter===key}"`)).join('')}</div></div>
    <div id="host-list">${filteredHosts()}</div>
    <div class="notice"><span class="notice-icon">${icon('bolt')}</span><div><strong>Обновляйте маршруты без остановки приложений</strong><p>Проверим конфигурацию перед применением. Если что-то пойдёт не так, автоматически вернём предыдущую версию.</p></div><span class="notice-tag">${t('SAFE RELOAD')}</span></div>`;
}
function filteredHosts() {
  if (!config.hosts.length) return empty(t('Первый домен — начало вашей сети'),t('Добавьте один или несколько доменов. Для каждого настройте собственные frontend, API и хранилище.'),`<div class="actions">${btn('add-host',t('Добавить домен'),'plus','primary')}${btn('example',t('Пример example.com'),'routes')}</div>`);
...
function settingsPage() {
  ...
  return ui`<form id="settings-form" class="stack"><section class="panel"><div class="panel-head"><h2>Ограничения на один IP-адрес</h2><p>Общий лимит действует суммарно на всех включённых доменах и маршрутах. При превышении — HTTP 429.</p></div><div class="grid">${field(t('Запросов в секунду'),'rate_rps',s.rate_rps,'number','min="0" max="100000" required',t('0 — без общего ограничения'))}${field(t('Допустимый всплеск (burst)'),'rate_burst',s.rate_burst,'number','min="0" max="100000" required',t('Дополнительные запросы проходят сразу; сверх burst — 429'))}${field(t('Одновременных запросов с IP'),'connections_per_ip',s.connections_per_ip,'number','min="0" max="65535" required',t('0 — без ограничения'))}${field(t('Максимальное тело запроса, MiB'),'body_mb',s.body_mb,'number','min="1" max="102400" required',t('Можно переопределить для каждого маршрута'))}</div></section><section class="panel"><div class="panel-head"><h2>Внешний SSL-прокси и DNS</h2><p>Nginx принимает обычный HTTP. Укажите IP вашего прокси, чтобы использовать реальный адрес клиента и X-Forwarded-Proto.</p></div><div class="grid"><label>Доверенные прокси, IP или CIDR<textarea name="trusted_proxies" rows="4" placeholder="172.20.0.2/32">${esc(s.trusted_proxies.join('\n'))}</textarea><small>По одному в строке. Доверяйте только своему SSL-прокси; он должен корректно передавать X-Forwarded-For.</small></label><label>DNS-серверы<textarea name="resolvers" rows="4" required>${esc(s.resolvers.join('\n'))}</textarea><small>В Docker-сети обычно 127.0.0.11. Имена upstream обновляются автоматически каждые 10 секунд.</small></label></div></section><section class="panel"><div class="panel-head"><h2>Базовая HTTP-защита</h2><p>TRACE и нестандартные методы запрещены. Включены таймауты чтения заголовков и тела запроса; версия Nginx скрыта.</p></div><div class="stack">${check(t('Запретить скрытые файлы и каталоги (.env, .git); разрешить .well-known'),'block_dotfiles',s.block_dotfiles)}${check(t('Добавить X-Content-Type-Options и Referrer-Policy'),'security_headers',s.security_headers)}</div><div class="notice">${icon('info')}<div>Это базовые ограничения Nginx. Для WAF и защиты от объёмных DDoS нужен отдельный внешний слой.</div></div></section><div class="actions"><button class="primary" type="submit">${icon('save')}Сохранить настройки в черновик</button>${btn('password',t('Сменить пароль панели'),'lock')}</div></form>`;
}
function bindSettings() {
  const form = $('#settings-form');
  form.addEventListener('input', () => {dirty = true; updateSaveState();});
  form.addEventListener('submit', event => {event.preventDefault(); if (!form.reportValidity()) return; task(async()=>{collectSettings(); await saveDraft(); notify(t('Настройки сохранены. Нажмите «Применить».'));});});
}
function collectSettings() {
  const form = $('#settings-form'); if (!form) return;
  if (!form.reportValidity()) throw new Error(t('Проверьте поля настроек'));
...
function cachePage() {
  ...
  return ui`<section class="panel"><div class="panel-head"><h2>Кеш ответов Nginx</h2><p>Общий диск: ${config.settings.cache_mb} MiB. Отдельный TTL для каждого маршрута. Кеш хранится в /cache.</p><div class="actions">${btn('cache-size',t('Изменить размер'),'edit','small')}</div></div><div class="cache-grid">${rs.map(({h,r})=>`<div class="cache-item"><div class="flex between"><h3>${esc(r.name)}</h3>${badge(r.cache ? t('Включён') : t('Выключен'),r.cache ? 'green':'gray')}</div><code>${esc(h.domain)}${esc(r.path)}</code><p>${r.cache ? ui`TTL ${r.cache_ttl} сек · GET / HEAD · статусы 200, 301, 302`:t('Ответы передаются напрямую от приложения.')}</p><div class="actions">${btn('edit-route',t('Настроить'),'edit','small',`data-host="${h.id}" data-route="${r.id}"`)}</div></div>`).join('')}</div>${rs.length ? '' : empty(t('Пока нет маршрутов'),t('Добавьте домен и маршрут, затем включите кеширование.'))}</section><div class="notice">${icon('shield')}<div><strong>Персональные ответы не кешируются.</strong><br>Authorization, Cookie, Set-Cookie, подписанные S3-параметры и WebSocket исключены. Учитываются Cache-Control и Vary приложения. Включайте кеш только для публичных ответов.</div></div>`;
}
function historyPage() {
  return ui`<section class="panel"><div class="panel-head"><h2>Применённые конфигурации</h2><p>Храним последние 20 версий. Восстановление загружает версию в черновик; применение выполняется отдельно.</p></div>${state.history.map(h=>ui`<div class="history-row"><div><div class="flex"><code>${esc(h.id.slice(0,12))}</code>${h.active ? badge(t('Действующая'),'green') : ''}</div><p>${new Date(h.applied_at * 1000).toLocaleString(locale())} · Доменов: ${h.hosts}</p></div>${btn('restore',t('В черновик'),'history','small',`data-generation="${h.id}"`)}</div>`).join('')}</section>`;
}
async function loadPreview(active=false){
  const el = $('#preview'); if (!el) return;
  try {
    if (active) {const response = await fetch('/api/active.conf'); if (!response.ok) throw new Error(t('Не удалось прочитать конфигурацию')); el.textContent = await response.text(); const heading = $('#preview-heading'); if (heading) { heading.textContent = t('Действующий конфиг'); heading.nextElementSibling.textContent = t('Применён'); }}
    else {el.textContent = (await api('preview',{config})).nginx;}
  } catch(error) {el.textContent = translateError(error.message);}
}
...
function updatePathPreview() {
  ...
  $('#path-preview').innerHTML = ui`<span>Пример запроса</span><code>${esc(path==='/' ? '/users' : path+'/users')}</code>${icon('arrow')}<code>${esc(stripped ? '/users' : path==='/' ? '/users' : path+'/users')}</code>`;
}
function editRoute(hostId, routeId) {
  ...
  setModal(existing ? route.name : t('Новый маршрут'),ui`Настройки маршрута для <strong>${esc(host.domain)}</strong>`, ui`
    <div class="route-tabs" role="tablist" aria-label="Настройки маршрута">${[['general',t('Маршрут'),'routes'],['servers',t('Серверы'),'server'],['advanced',t('Кеш и лимиты'),'sliders']].map(([key,label,symbol])=>btn('route-tab',label,symbol,key==='general' ? 'selected' : '',`data-tab="${key}" role="tab" id="tab-${key}" aria-controls="panel-${key}" aria-selected="${key==='general'}" tabindex="${key==='general' ? 0 : -1}"`)).join('')}</div>
    ...
    <section data-route-panel="advanced" id="panel-advanced" role="tabpanel" aria-labelledby="tab-advanced" hidden>
      ...
      <div class="option-list">${check(t('<span><strong>Кешировать публичные ответы</strong><small>Только GET/HEAD без cookies и авторизации.</small></span>'),'cache',route.cache)}</div>
      <div class="grid">${field(t('TTL кеша, сек'),'cache_ttl',route.cache_ttl,'number','required min="1" max="604800"')}${field(t('Макс. тело запроса, MiB'),'body_mb',route.body_mb,'number','required min="0" max="102400"',t('0 — использовать общий лимит'))}${field(t('Дополнительный лимит, r/s'),'rate_rps',route.rate_rps,'number','required min="0" max="100000"',t('0 — только общий лимит на IP'))}${field(t('Допустимый всплеск (burst)'),'rate_burst',route.rate_burst,'number','required min="0" max="100000"')}</div>
      ${route.path.includes('s3') ? ui`<div class="inline-info">${icon('info')}<span>Для S3 подпись зависит от Host и пути. Удаление префикса может нарушить SigV4. Для стандартного S3 API удобнее отдельный домен.</span></div>` : ''}
    </section>`, async data=>{
...
function example() {
  ...
  notify(t('Пример добавлен. Укажите реальные адреса серверов перед применением.'));
}
  ...
  try {
    ...
    if (action === 'delete-host') {
      ...
      if (host && await confirmAction(t('Удалить домен?'), t('Будут удалены домен и все его маршруты.'), host.domain, t('Удалить домен'), true)) {
        ...
        notify(t('Домен удалён из редактора. Сохраните черновик или примените изменения.'));
      }
    }
    if (action === 'delete-route') {
      const host = config.hosts.find(h=>h.id === button.dataset.host);
      const route = host?.routes.find(r=>r.id === button.dataset.route);
      if (route && await confirmAction(t('Удалить маршрут?'), ui`Маршрут «${route.name}» будет удалён.`, host.domain + route.path, t('Удалить маршрут'), true)) {
        host.routes = host.routes.filter(r=>r.id !== route.id); $('#modal').close(); changed();
        notify(t('Маршрут удалён из редактора. Сохраните черновик или примените изменения.'));
      }
    }
    if (action === 'save') await task(async()=>{await saveDraft(); notify(t('Черновик сохранён'));});
    if (action === 'apply') await task(async()=>{await saveDraft(); accept(await api('apply',{revision:state.revision})); notify(t('Конфигурация проверена и применена'));});
    if (action === 'test') await task(async()=>{await saveDraft(); const result=await api('test',{revision:state.revision}); notify(result.output);});
    if (action === 'active-config') {await loadPreview(true); notify(t('Показан действующий nginx.conf'));}
    if (action === 'copy-config') {const text=$('#preview').textContent; if (navigator.clipboard && window.isSecureContext) {await navigator.clipboard.writeText(text); notify(t('Скопировано'));} else download('nginx.conf',text,'text/plain');}
    if (action === 'restore' && (!dirty || await confirmAction(t('Загрузить версию?'), t('Несохранённые изменения будут заменены выбранной версией.'), '', t('Загрузить версию')))) await task(async()=>{accept(await api('restore',{generation:button.dataset.generation,revision:state.revision})); notify(t('Версия загружена в черновик. Нажмите «Применить».'));});
    if (action === 'export') {collectSettings(); download('ambergate-config.json',JSON.stringify(config,null,2),'application/json');}
    if (action === 'import') {const input=document.createElement('input'); input.type='file'; input.accept='.json,application/json'; input.addEventListener('change',()=>task(async()=>{const file=input.files[0]; if(!file)return; if(file.size>1048576)throw new Error(t('Файл больше 1 MiB')); const candidate=JSON.parse(await file.text()); await api('preview',{config:candidate}); if(config.hosts.length && !await confirmAction(t('Импортировать конфигурацию?'), t('Текущие настройки редактора будут заменены конфигурацией из файла.'), file.name, t('Импортировать')))return; config=candidate; dirty=true; notify(t('Импортировано в редактор. Проверьте настройки и примените.'));}));input.click();}
    if (action === 'cache-size') setModal(t('Размер кеша'),t('Общий лимит дискового кеша для всех маршрутов.'),field(t('Размер, MiB'),'cache_mb',config.settings.cache_mb,'number','required min="1" max="1048576"'),data=>{config.settings.cache_mb=Number(data.get('cache_mb'));changed();});
    if (action === 'password') setModal(t('Новый пароль'),t('После изменения потребуется войти снова.'),`<div class="stack">${field(t('Текущий пароль'),'current','','password','required autocomplete="current-password"')}${field(t('Новый пароль'),'password','','password','required minlength="12" maxlength="256" autocomplete="new-password"')}</div>`,async data=>{await api('password',{current:data.get('current'),password:data.get('password')});csrf='';showLogin();notify(t('Пароль изменён. Войдите снова.'));},t('Изменить пароль'));
...
document.addEventListener('keydown', event=>{
  ...
  if((event.ctrlKey || event.metaKey) && event.key.toLowerCase()==='s' && config && !$('#modal').open) {event.preventDefault();if(dirty) task(async()=>{await saveDraft();notify(t('Черновик сохранён'));});}
});
