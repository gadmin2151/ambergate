...
function agentCompose(token, origin, network='host', allowHttp=false, namespace=false) {
  checkAgentInstall(origin,network,allowHttp);
  if(namespace && network!=='host')throw new Error(at('Для namespace выберите сеть хоста','Namespace access requires host networking'));
  return `services:\n  ambergate-agent:\n    image: ghcr.io/gadmin2151/ambergate-agent:latest\n    pull_policy: always\n    container_name: ambergate-agent\n    restart: unless-stopped\n    init: true\n    read_only: true\n    tmpfs:\n      - /tmp:size=16m,mode=1777\n${namespace?'    pid: host\n    cap_add: [SYS_ADMIN, SYS_PTRACE]\n':''}    environment:\n${namespace?'      AMBERGATE_AGENT_NAMESPACE: \"true\"\n':''}      AMBERGATE_DOCKER_CONTAINER: ambergate-agent\n      AMBERGATE_SERVER_URL: ${JSON.stringify(origin)}\n      AMBERGATE_AGENT_TOKEN: ${JSON.stringify(token)}\n${origin.startsWith('http://')?'      AMBERGATE_AGENT_ALLOW_HTTP: "true"\n':''}    volumes:\n      - type: bind\n        source: /var/run/docker.sock\n        target: /var/run/docker.sock\n        read_only: true\n        bind:\n          create_host_path: false\n${network==='host'?'    network_mode: host\n':'    networks:\n      - applications\n'}    logging:\n      driver: json-file\n      options:\n        max-size: 10m\n        max-file: "3"\n${network==='host'?'':`networks:\n  applications:\n    external: true\n    name: ${JSON.stringify(network)}\n`}`;
}
const shellQuote = value => "'" + String(value).replaceAll("'", "'\"'\"'") + "'";
function agentDockerRun(token, origin, network='host', allowHttp=false, namespace=false) {
  checkAgentInstall(origin,network,allowHttp);
  if(namespace && network!=='host')throw new Error(at('Для namespace выберите сеть хоста','Namespace access requires host networking'));
  return `docker run -d --pull always --name ambergate-agent --restart unless-stopped --init --read-only --tmpfs /tmp:size=16m,mode=1777 --network ${shellQuote(network)}${namespace?' --pid host --cap-add SYS_ADMIN --cap-add SYS_PTRACE -e AMBERGATE_AGENT_NAMESPACE=true':''} -e AMBERGATE_DOCKER_CONTAINER=ambergate-agent --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock,readonly -e ${shellQuote('AMBERGATE_SERVER_URL='+origin)} -e ${shellQuote('AMBERGATE_AGENT_TOKEN='+token)}${origin.startsWith('http://')?' -e AMBERGATE_AGENT_ALLOW_HTTP=true':''} --log-driver json-file --log-opt max-size=10m --log-opt max-file=3 ghcr.io/gadmin2151/ambergate-agent:latest`;
}
...
function agentSetup(token, identity, name, connection, rotating=false) {
  ...
  let format='run';
  modal.innerHTML=`<div class="modal-header"><div><span class="eyebrow">AMBERGATE AGENT</span><h2 id="agent-setup-title">${at('Подключите машину','Connect your machine')}</h2><p class="modal-subtitle">${esc(name)}</p></div>${btn('close','','close','ghost icon',`aria-label="${t('Закрыть')}"`)}</div><div class="modal-body"><ol class="agent-steps"><li class="complete"><span>1</span>${at('Агент создан','Agent created')}</li><li class="active"><span>2</span>${at('Запустите команду','Run the command')}</li><li id="agent-connect-step"><span>3</span>${at('Подключение','Connected')}</li></ol><div class="connection-address ${insecure?'insecure':''}">${icon(insecure?'info':'lock')}<div><strong>${esc(origin)}</strong><p>${insecure?at('HTTP разрешён в общих настройках. Токен и трафик не зашифрованы.','HTTP was allowed in General settings. Token and traffic are unencrypted.'):at('Адрес из общих настроек · HTTPS с проверкой сертификата','From General settings · HTTPS with certificate verification')}</p></div></div><div class="agent-install-network"><label>${at('Доступ к контейнерам','Container access')}<select id="agent-install-mode"><option value="host">${at('Сеть хоста · локальные bridge-сети (Linux)','Host network · local bridge networks (Linux)')}</option><option value="namespace">${at('Расширенный доступ · namespace (Linux)','Extended access · namespace (Linux)')}</option><option value="custom">${at('Отдельная Docker-сеть','Specific Docker network')}</option></select></label><p id="agent-host-hint" class="hint">${at('Сеть создавать не нужно. Агент использует внутренние IP контейнеров. Для обычного Docker Engine на Linux; для Docker Desktop выберите отдельную сеть.','No network creation needed. The agent uses private container IPs. For native Docker Engine on Linux; for Docker Desktop, choose a specific network.')}</p><p id="agent-namespace-hint" class="docker-info" hidden>${at('Доступ к сетевому пространству выбранного контейнера, включая localhost и network=none. Требуются PID хоста и права SYS_ADMIN / SYS_PTRACE. Приложения и их сети не изменяются.','Access the selected container’s network namespace, including localhost and network=none. Requires host PID access and SYS_ADMIN / SYS_PTRACE capabilities. Applications and their networks stay unchanged.')}</p><div id="agent-custom-network" hidden>${field(at('Docker-сеть приложений','Application Docker network'),'network','','text','id="agent-install-network" maxlength="128" spellcheck="false" placeholder="my-app_default"',at('Существующая сеть на машине агента. Для нескольких сетей подключите агент к каждой.','An existing network on the agent machine. For multiple networks, attach the agent to each.'))}</div></div>${rotating?`<p class="docker-info">${at('Старый токен отключён. Пересоздайте существующий агент с новой командой или замените Compose-файл и выполните docker compose up -d.','The old token is revoked. Recreate your existing agent with the new command, or replace the Compose file and run docker compose up -d.')}</p>`:''}<div class="agent-install-tabs" role="tablist" aria-label="${at('Способ запуска','Installation method')}"><button type="button" id="agent-tab-run" role="tab" aria-controls="agent-install-panel" aria-selected="true" data-install-format="run">${icon('code')}Docker run</button><button type="button" id="agent-tab-compose" role="tab" aria-controls="agent-install-panel" aria-selected="false" tabindex="-1" data-install-format="compose">${icon('docker')}Docker Compose</button></div><div id="agent-install-panel" role="tabpanel" aria-labelledby="agent-tab-run"><div class="agent-command-head"><span id="agent-install-caption"></span><button type="button" id="agent-copy-command" class="small">${icon('copy')}<span>${at('Копировать команду','Copy command')}</span></button></div><pre id="agent-install-code" tabindex="0"></pre><div id="agent-compose-launch" hidden><p>${at('Скачайте файл и выполните в его каталоге:','Download the file, then run in its directory:')}</p><code>docker compose -f compose.agent.yaml up -d</code><button type="button" id="agent-download-compose" class="primary">${icon('download')}${at('Скачать Compose','Download Compose')}</button></div></div><div id="agent-setup-error" class="error" role="alert"></div><p class="hint">${at('Команда и файл содержат персональный токен. Он доступен только в этом окне — сохраните нужный вариант перед закрытием.','The command and file contain a personal token. It is available only in this window — save your preferred option before closing.')}</p><div id="agent-install-status" class="agent-install-status" role="status" aria-live="polite"></div></div><div class="form-actions"><span>${at('Подключение появится автоматически','Connection appears automatically')}</span>${btn('close',at('Готово','Done'),'check','primary')}</div>`;
  const selectedNetwork=()=>$('#agent-install-mode').value==='custom'?$('#agent-install-network').value.trim():'host';
  const namespaceAccess=()=>$('#agent-install-mode').value==='namespace';
  const renderCode=()=>{
    const code=$('#agent-install-code'), host=$('#agent-install-mode').value!=='custom';
    $('#agent-custom-network').hidden=host;$('#agent-host-hint').hidden=!host || namespaceAccess();$('#agent-namespace-hint').hidden=!namespaceAccess();
    $('#agent-install-network').required=!host;
    let value='';
    try {
      value=format==='run'?agentDockerRun(token,origin,selectedNetwork(),insecure,namespaceAccess()):agentCompose(token,origin,selectedNetwork(),insecure,namespaceAccess());
  ...
  $('#agent-install-mode').addEventListener('change',renderCode);
  ...
  $('#agent-download-compose').addEventListener('click',()=>{
    download('compose.agent.yaml',agentCompose(token,origin,selectedNetwork(),insecure,namespaceAccess()),'application/yaml');
  });
