...
function agentCompose(token, origin, network='host', allowHttp=false) {
...
function agentDockerRun(token, origin, network='host', allowHttp=false) {
  checkAgentInstall(origin,network,allowHttp);
  return `docker run -d --pull always --name ambergate-agent --restart unless-stopped --init --read-only --tmpfs /tmp:size=16m,mode=1777 --network ${shellQuote(network)} -e AMBERGATE_DOCKER_CONTAINER=ambergate-agent --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock,readonly -e ${shellQuote('AMBERGATE_SERVER_URL='+origin)} -e ${shellQuote('AMBERGATE_AGENT_TOKEN='+token)}${origin.startsWith('http://')?' -e AMBERGATE_AGENT_ALLOW_HTTP=true':''} --log-driver json-file --log-opt max-size=10m --log-opt max-file=3 ghcr.io/gadmin2151/ambergate-agent:latest`;
}
...
function agentSetup(token, identity, name, connection, rotating=false) {
  ...
  $('#agent-download-compose').addEventListener('click',()=>{
    download('compose.agent.yaml',agentCompose(token,origin,selectedNetwork(),insecure),'application/yaml');
  });
