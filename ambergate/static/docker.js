  ...
  try {
    ...
    if(dockerPickerSource==='local') {
      ...
      data={agent,settings:{enabled:true,host_address:''},mode:'agent',connected:ready,containers:agent.inventory.map(c=>({
        ...c,namespace_access:c.endpoints.some(e=>e.kind==='namespace'),endpoints:[{address:c.name,kind:'agent'}],
        selectable:ready && agent.manual_routing && c.state==='running' && !c.is_gateway && c.endpoints.length>0 && (c.shared_networks.length>0 || c.endpoints[0].kind==='namespace'),
