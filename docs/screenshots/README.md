...
Captured from AmberGate revision `bc51831` on 23 September 2026 using Chromium. These are actual UI screenshots from an isolated local instance, not design mockups. Both English and Russian captures are included.
...
| `routes.en.png`, `routes.ru.png` | Domains, paths, targets, balancing and caching |
| `rewriting.en.png`, `rewriting.ru.png` | Route response rewriting and a generic `{prefix}` substitution |

To refresh the images, start an isolated instance with disposable credentials and example domains. Generate test requests through its Nginx listener, sign in, and capture each language at a 1440 × 1100 viewport with device scale 1. For the rewriting screen, open the route, expand additional HTML substitutions and capture the modal. Review every image for private data before committing.

---

Скриншоты сняты в Chromium с настоящего интерфейса версии `bc51831` на отдельном локальном стенде, 23 сентября 2026 года. Используются примерные домены и временные HTTP-приложения. Показатели получены из тестовых запросов через Nginx: это **демонстрация интерфейса, а не измерение производительности**. Производственные адреса, пароли и токены не используются.
