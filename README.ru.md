...
Неверные labels, противоречивые настройки, неполный список контейнеров или недоступный Docker приостанавливают всё обновление, сохраняя последнюю рабочую конфигурацию. Ограничения: 16 labels на контейнер, 4096 символов на label, 32 сервера на маршрут, 50 маршрутов на домен, 100 доменов. Список из 500 контейнеров считается неполным. Всё хранится локально: `/data/docker.json`, `/data/labels.json`, черновики и версии конфигурации. Отключение labels или Docker сохраняет текущие маршруты без дальнейшего обновления. При включённой автоматизации право создавать контейнеры с labels на этом Docker daemon позволяет влиять на маршрутизацию; используйте доверенные контейнеры.
