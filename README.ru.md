...
<p align="center"><a href="#быстрый-старт">Быстрый старт</a> · <a href="#что-умеет">Возможности</a> · <a href="#карта-сервисов">Карта сервисов</a> · <a href="#удалённые-агенты">Агенты</a> · <a href="#переписывание-ответов">Переписывание ответов</a> · <a href="#документация">Документация</a></p>

**AmberGate** — HTTP gateway на базе Nginx, панели на Python и обычного JavaScript. Центральный gateway и панель работают в **одном Docker-контейнере**. Для каждой удалённой Docker-машины можно добавить один контейнер агента. Внешняя база данных и отдельный сервис метрик не нужны; можно оставить внешний TLS-прокси или включить Let’s Encrypt отдельно для домена.
...
> **v0.2.0:** ограничение запросов панели, квоты туннелей на агента и корректный вход через доверенный прокси. [Обновление](CHANGELOG.md#020--2026-09-23) · [Защита панели](docs/control-plane-security.ru.md)
...
| **[Удалённые агенты](#удалённые-агенты)** | Исходящие HTTPS/WSS-туннели, ручной выбор контейнеров и балансировка между машинами без публикации портов |
...
## Домены и необязательный SSL

Раздел **Маршруты** показывает все домены плитками. Выберите домен, чтобы перейти к его маршрутам, целевым серверам и SSL-настройкам.

![Обзор доменов](docs/screenshots/domains.ru.png)

**Let’s Encrypt по умолчанию выключен.** Включите его в настройках домена, укажите email и примените изменения. Сертификат автоматически продлевается **за 5 дней до истечения**; срок можно изменить отдельно для домена. Даты и ошибки выпуска обновляются через SSE. Перенаправление HTTP → HTTPS включается отдельно.

Для HTTPS опубликуйте порт 443 через `compose.tls.yaml` или `first-start.sh --with-tls`. Публичный порт 80 должен вести на gateway для HTTP-01. [Включение, продление и работа за внешним прокси →](docs/ssl.ru.md)
...
Настройки самого приложения не меняются. Режим работает с прямыми серверами и туннелями агентов, в том числе для HTTPS-клиентов с SSL от AmberGate или внешнего TLS-прокси. Раздел **Дополнительные замены в HTML** позволяет описать строки конкретного приложения через общий редактор; `{prefix}` подставляет путь маршрута.
...
Что к чему подключается: внешний TLS-прокси, центральный AmberGate, локальные приложения и удалённые Docker-машины. Пунктир показывает исходящие подключения агентов; трафик приложений идёт по созданным ими туннелям.

[![Карта подключений сервисов AmberGate](docs/diagrams/service-map.ru.png)](docs/diagrams/service-map.ru.png)

<details>
<summary><strong>Проследить путь запроса и обратного ответа</strong></summary>

[![Запрос через AmberGate, агент и приложение с переписыванием ответа](docs/diagrams/request-flow.ru.png)](docs/diagrams/request-flow.ru.png)

</details>

Это схемы примерного развёртывания, а не живая карта конкретного сервера. **[Соединения, порты и векторные версии →](docs/architecture.ru.md)**

<a id="удалённые-docker-машины"></a>
...
1. Агент читает локальный **`docker.sock`**: находит контейнеры, их состояние, внутренние порты, сети и маршрутные labels. По умолчанию он отправляет свежий список в центр каждые **5 секунд**. Переменные окружения, mounts и посторонние labels в отчёт не входят.
2. Агент **сам подключается к центру** по HTTPS/WSS с отдельным токеном. На удалённой машине не нужен входящий порт: достаточно доступа к адресу центра и к выбранным приложениям. Центральному AmberGate не требуется прямой доступ к Docker socket или IP этой машины.
3. В панели вы выбираете контейнеры для маршрута вручную или включаете автоматизацию по **`ambergate.route`**. Центр формирует и проверяет конфигурацию Nginx; агент обеспечивает соединение с приложением.
4. Когда приходит запрос, Nginx выбирает целевой контейнер, а агент доставляет ему запрос через туннель. Ответ возвращается тем же путём. Поддерживаются потоковые загрузки, SSE и WebSocket приложений.

```text
Клиент → центральный Nginx → туннель WSS → агент → HTTP-порт контейнера
Клиент ← центральный Nginx ← туннель WSS ← агент ← ответ приложения
```

Туннель открывает агент, хотя пользовательский запрос идёт от центра к приложению. HTTPS для подключения агента завершает внешний TLS-прокси перед панелью **8083**; SSL доменов приложений настраивается отдельно.

### Что это даёт

| Преимущество | На практике |
|:--|:--|
| **Работа за NAT** | Подключайте машины из офиса, домашней сети или другого дата-центра. Для этого туннеля не нужны проброс портов и отдельный VPN, если агенту доступен центр |
| **Приватные порты приложений** | Backend может слушать только внутри Docker: публиковать `3000`, `8080` и другие порты на хосте не требуется |
| **Одна панель для всех машин** | Удалённые контейнеры доступны в том же редакторе маршрутов, что и локальные; домены, кеш и ограничения задаются централизованно |
| **Балансировка между серверами** | Объединяйте контейнеры разных агентов и локальные цели в одном маршруте; задавайте веса, алгоритм и резервные серверы |
| **Меньше ручных правок** | Labels позволяют добавлять новые реплики в балансировщик. При пересоздании вручную выбранного контейнера с прежним именем его адрес обновится после следующего отчёта |
| **Доступ к разным Docker-сетям** | Выберите сеть хоста или расширенный namespace-режим в зависимости от изоляции приложений; варианты и права описаны ниже |

**Пример:** две машины за NAT, на каждой агент и backend на внутреннем порту `3000`. Задайте обоим backend одинаковый label `ambergate.route: "host=example.com; path=/api; port=3000; strip=true"` и включите **Применять автоматически**. AmberGate создаст один маршрут `/api` с двумя целями на разных машинах — без публикации порта `3000`.

При обрыве связи агент автоматически переподключается, а панель показывает его состояние. Уже открытые соединения при обрыве завершаются. Для маршрута с несколькими целями Nginx может попробовать другую цель согласно настройкам повторных попыток; туннель не заменяет проверку готовности самого приложения.

### Подключение за несколько шагов

1. Сохраните адрес центра в **Общих настройках**, например `ambergate.exemple.com`.
...
|:--|:--|
| [Карта сервисов и путь запроса](docs/architecture.ru.md) | Схемы соединений, роли компонентов, порты и туннели |
| [Необязательный SSL](docs/ssl.ru.md) | Let’s Encrypt, автопродление, порты и хранение сертификатов |
