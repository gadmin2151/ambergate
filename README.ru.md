...
**Расширенный доступ:** дополнительный namespace-режим подключается к приватным портам даже в `network=none` и к приложениям на localhost. Один контейнер агента, без изменения сетей приложений. Нужные права Linux описаны в руководстве.
...
Включите **Сохранять базовый путь в ответах** в маршруте: перенаправления, пути cookies и ссылки HTML будут использовать префикс вроде `/api5`. Дополнительные замены HTML настраиваются через интерфейс. Работает с прямыми серверами и агентами без изменений самого сервиса. [Возможности и ограничения](docs/response-rewriting.ru.md).
